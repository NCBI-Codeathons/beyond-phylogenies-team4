...
19. Preliminarily add samples to all previous trees using UShER in order to evaluate thir best-fitting cluster origin (creating new temporary pb files for today's date). *NOTE*: you do not need both the -p -o argument. Output MAT files (-o) will not be created if parsimony placements (-p) is called.

	```
	for i in $(ls | grep "${previous}.pb");
	do usher -i ${i} -v ${run}_${today}/${run}_florida_gisaid_${today}_masked.vcf -p -o ${i%_${previous}.pb}_${today}_prelim.pb; done	
	```
