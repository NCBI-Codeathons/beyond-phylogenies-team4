...
1. Pull flaco dependencies from repo:
...
5. Meanwhile, create directory for initial sequencing run to initiate sub-project:
...
6. Extract sequences from desired region from database that correspond to the relevant time frame:

	```
	gisaidfilt.py ../gisaid_msa/msa_${MSADATE}/msa_${MSADATE}.fasta -n USA/FL -a ${DATE1}_00 -b ${DATE2}+00  -o florida_${today}.fasta;
	```

7. Combine in-house sequences with GISAID Floridian sequences:
	
	```
	cat ${run}_${today}.fasta florida_${today}.fasta > ${run}_florida_${today}.fasta;
	```

8. Remove Floridian sequences from GISAID database, which results in a new "clean.fa" gisaid database:	
...
9. Strip sequences of gaps (improves BLAST performance):
...
13. Perform multiple sequence alignment using viralmsa:
	...
	**If no step prior to workflow exclude low-quality sequences (specifically high N content, proceed with Steps 14-16, else proceed to Step 17.**

14. Check for extensive gaps in sequences (viralmsa can sometimes result in extensive gaps if number of ambiguous sites is too high). This step does not necessarily have to be automated if earlier step filters out low-quality sequences.
...
18. Change first sequence header to just MN908947.3 (spaces in original name wreak havoc downstream):
...
20. Merge metadata with pangolin file containing minimal information used for DYNAMITE/PHYLOPART (*May need to modify so that column names are ID and DATE and first two columns):
...
22. Add reference sequence again to each of the newly generated fasta files:
...
24. Transform fasta sub-alignments to vcf
	
	```
	for i in ./*.fasta; do ./faToVcf -ref=MN908947.3 ${i} ${i%.fasta}.vcf; done
	```
	
25. Perform subtree pre-processing (need to modify to automatically only select cluster and background trees and to exclude original, full tree) 
...
## Wash, rinse, repeat
26. Update dates.txt file with new date and assign current ("today") and previous ("previous") date variables and follow steps above:
...
27. Preliminarily add samples to all previous trees using usher (creating new pb files for today's date):
...
28. Compute parsimony score for new samples (vcf) assigned to each annotated tree (.pb files):
...
29. Previous step will generate parsimony scores in tsv files, which we need to rename and copy to single folder for R analysis:
...
30. Use R script to evaluate parsimony scores from tsv files and output new fasta files for sequences needing to be placed on trees (currently not working owing to change in usher output):
...
31. Add reference sequence again to each of the newly generated fasta files (from previous step):
	
	```
	for i in ./*.fasta; do cat ../cov_reference/cov_reference.fasta >> ${i}; done
	```

32. Place updated fasta in new folders:
...
33. Replace old folders in source.txt file with updated folders (noticed old source.txt file generation step is missing...):
...
34. Convert to vcf as above
	
	 ```
	for i in $(cat source.txt); do faTovcf -ref=MN908947.3 ${i}/*.fasta ${i}/*.vcf; done	
	```

35. Officially add samples to all old trees using usher (creating new pb files for today):
...
36. Not all trees (.pb) are going to be updated with sequences, so need to find all trees without today's date and make a copy of previous pb with today's date:
...
37. Move new unannotated tree (.nh) files generated from the previous step into one folder (as well as original combined sample fasta) for characterization:
...
38. Run modified DYNAMITE to search for (and prune) clusters within background tree: *Remind Brittany to modify dynamite to make cluster names in sequential order and if cluster already exists in other folder, add new digit place*
...
43. Concatenate previous fasta with updated fasta for tree optimization :
...
45. Force bifurcating tree in R:
...
46. Re-optimize trees (only those that were updated) using FastTree: 
...
47. Re-annotate updated fasttree trees (replace old ones):
...
48. Test background and discard trees for clusters (separate R script using actual branchwise algorithm). R script will write new results for discard pile to a new folder, but will write results from background to same folder.
...
49. Process fasta and tree for background (will require determining if new files added as a result of R script in step above)
...
50. Add reference sequence to new cluster fastas and place in new directories:
	
	```
	for i in ./*.fasta; do  cat ./cov_reference/cov_reference.fasta >> ${i}; cp ${i} .${i%.fasta}; done
	```
	
51. Update source.txt file with new clusters and process fastas:
...
52. Perform subtree pre-processing for new clusters:
