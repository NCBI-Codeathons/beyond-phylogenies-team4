...
print("Checking for multiple sequence alignment file...")
fa <- read.FASTA(opt$fasta)

print("Checking for tree file...")
...
if(opt$timetree=="Y") {
  ...
  sts <- checkForDates(metadata)
    ...
    if(isTRUE(length(sub_tree$tip.label) <= 30000)) {
      ...
      #outliers <- invisible(outlierTips(result, alpha=0.01))
      #outliers <- outliers$taxon[outliers$p<0.001]

      #print(paste0("The following sequence has been identified as temporal outliers:\n", outliers))

      #metadata <- filter(metadata, ID %notin% outliers)
      #assign("metadata", metadata, envir=globalenv())

	  sub_tree <- result$intree
	  #sub_tree <- drop.tip(result$intree, outliers)
	  assign("sub_tree", sub_tree, envir=globalenv())

	  time_tree <- as.phylo(as_tibble(result))
	  #time_tree <- drop.tip(as.phylo(as_tibble(result)), outliers)
	  assign("time_tree", time_tree, envir=globalenv())

	  #fa <- fa[names(fa) %notin% outliers]
...
print("Defining and extracting well-supported clades within the tree using node labels...")
define.clades <- function(sub_tree) {
  ...
  clades <- mclapply (unique(supported_nodes$to), function(node) {
    familyR::get_children(family_tree, node)
  }, mc.cores=numCores) # End loop along family_tree_supported
  ...
  clades <- mclapply (seq_along(clades), function(i) {
    merge(clades[[i]]$edges,clades[[i]]$nodes, by.x = "to", by.y = "id")
  }, mc.cores=numCores) # End loop along clades
...
print("ASR performed using the empirical Bayesian method.")
  ancStateRecon <- function(tree, metadata) {
    ...
    print("Estimating likelihood of ancestral states...", stderr())
    feed.mode <- list()
    fitER <- list()
  ...
  if (opt$asr == "N"){
  print("ASR not performed - results will rely on sampled data only.")
} else {
  print("Correct response not detected.")
}
}
  

# Data manipulation for characterization of clusters and export of data for visualization
print("Performing quick data manipulation... Hold on to your butts!")
...
#   psi <- rnorm(s, 9, 4) #Duration of infection around 14 days -incubation period of 5 days
