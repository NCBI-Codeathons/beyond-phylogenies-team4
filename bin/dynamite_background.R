...
option_list = list(
  make_option(c("-f", "--fasta"), type="character", default=list.files(pattern="*.fasta")[[1]], 
              help="fasta file name [default= .fasta extension]", metavar="character"),
  make_option(c("-t", "--tree"), type="character", default=list.files(pattern = "*.tree")[[1]], 
...
# metadata$ID <- gsub("(NC_045512.2).+", "\\1", metadata$ID)
# 
