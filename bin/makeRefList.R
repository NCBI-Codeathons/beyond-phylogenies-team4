...
print("Extracting whole genome reference sequences.")

print("Grouping reference sequences based on lineage and reformatting fasta files.")
r_reform <- list()
invisible(foreach (i=1:nrow(ref)) %do% {
  r_reform[[i]] <- as.character(ref[i,])[1,]
})
r_reform <- data.frame(do.call(rbind, r_reform))
rownames(r_reform) <- rownames(ref)
...
delta <- r_reform2[grepl("B.1.617.2|AY*", names(r_reform2))]

mclapply(seq_along(delta), function(x) {
write.FASTA(delta[[i]], paste0(names(delta)[[i]], "_reference.fasta"))
}, mc.cores=numCores)
